...
func Setup(cfg *config.Config, db *gorm.DB, hub *websocket.Hub) *gin.Engine {
	...
	v1 := r.Group("/api/v1")
	{
		...
		supplierPortal.Use(middleware.TenantMiddleware())
		{
			...
			supplierPortal.POST("/logout", supplierPortalHandler.Logout)
		...
		admin.Use(middleware.AdminAuthMiddleware(db, authService))
		{
			...
			admin.GET("/payments", middleware.RequireAdminPermission("billing:read"), adminHandler.ListPayments)
			admin.POST("/payments", middleware.RequireAdminPermission("billing:write"), adminHandler.CreatePayment)
			admin.GET("/payments/:id", middleware.RequireAdminPermission("billing:read"), adminHandler.GetPayment)
			admin.PUT("/payments/:id", middleware.RequireAdminPermission("billing:write"), adminHandler.UpdatePayment)
			admin.DELETE("/payments/:id", middleware.RequireAdminPermission("billing:write"), adminHandler.DeletePayment)
